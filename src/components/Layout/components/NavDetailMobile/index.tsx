...
const NavDetailMobile: FC<INavDetailMobile> = () => {
  const overflowMenu = storeSelector((state) => state.overflowMenu);
  const categories = storeSelector((state) => state.categories);
  ...
  useEffect(() => {
    ...
    storeSelector.getState().setOverflowMenu(false);
  }, [isOpen]);
